...
import { MapPin, Route, CheckCircle2, Navigation } from "lucide-react"
        ...
        try {
          ...
          if (rotaData && rotaData.waypoints) {
            ...
            setParticipantes(prev => {
              const prevKey = prev.map(p => `${p.id}-${p.checkedIn}-${p.coletado}`).join('|')
              const newKey = participantesOrdenados.map(p => `${p.id}-${p.checkedIn}-${p.coletado}`).join('|')
